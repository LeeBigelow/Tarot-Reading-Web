...
for layout in $layouts; do
	...
	# get number cards from "card" lines in html-stub
	num_cards=$(grep 'class="card"' "$htmld/layout-$layout.html" | wc -l)
	...
	# html for layout
	printf "${layout}_html = \`\n" >> $js_file
	sed 's/^/\t/' "$htmld/layout-${layout}.html" >> $js_file
	printf "\`;\n\n" >> $js_file

	# css for layout
	printf "${layout}_css = \`\n" >> $js_file
	sed 's/^/\t/' "$cssd/layout-${layout}.css" >> $js_file
	printf "\`;\n\n" >> $js_file
done
